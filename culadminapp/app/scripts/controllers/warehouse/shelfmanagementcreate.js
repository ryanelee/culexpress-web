  ...
  .controller('WarehouseShelfManagementCreateCtrl', ['$scope', '$location', '$window', 'warehouseService', 'shelfService', 'plugMessenger',
      function ($scope, $location, $window, warehouseService, shelfService, plugMessenger) {
          ...
          for (var i = 1; i < 1000; i++) {
              if (i < 10) {
                  $scope.shelfOptions.layer.push({ title: i.toString(), value: i.toString() });
              }
              if (i < 100) {
                  var val = i.toString().length < 2 ? "0" + i.toString() : i.toString();
                  $scope.shelfOptions.row.push({ title: val, value: val });
              }
              if (i < 1000) {
                  var val = "";
                  ...
                  $scope.shelfOptions.position.push({ title: val, value: val });
              }
          }
          ...
          $scope.btnPrint = function () {
              $scope.$broadcast("print-helper.action", "shelf-management-tag", { shelfNumber: _formatShelfNumber() });
          }
      }]);
