      ...
      self.getList = function (options, callback) {
          $http.post(cul.apiPath + "/warehouse/shelf/list", options).success(function (result) {
              _.each(result.data, function (item) {
                  switch (item.type) {
                      case "A": item._type = "海淘客户"; break;
                      case "B": item._type = "大客户"; break;
                  }
              });
              callback(result);
          })
          ...
          $http.post(cul.apiPath + "/item/transport/list", options).success(function (result) {
              _.each(result.data, function (item) {
                  switch (item.sendType) {
                      case 1: item._sendType = "寄送库存"; break;
                      case 2: item._sendType = "海淘包裹"; break;
                  }
                  switch (item.inboundStatus) {
                      case 0: item._inboundStatus = "等待登记"; break;
                      case 1: item._inboundStatus = "等待清点"; break;
                      case 2: item._inboundStatus = "部分收货"; break;
                      case 3: item._inboundStatus = "已收货"; break;
                      default: item._inboundStatus = "所有"; break;
                  }
                  switch (item.shelfStatus) {
                      case 0: item._shelfStatus = "未上架"; break;
                      case 1: item._shelfStatus = "部分上架"; break;
                      case 2: item._shelfStatus = "已上架"; break;
                  }
              });
              callback(result);
          });
