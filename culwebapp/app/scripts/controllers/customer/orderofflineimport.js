...
angular.module('culwebApp')
    .controller('OrderOfflineImportCtrl', ["$scope", "$timeout", "$filter", "orderService", "warehouseService",
                ...
                $wizard.on('change', function (e, data) {
                    if (data.direction == "next") {
                        if (!$scope.stepValidation[data.step - 1]) {
                            switch (data.step) {
                                case 1:
                                    alertify.alert("提示", "请上传线下订单Excel文件");
                                    break;
                                case 2:
                                    alertify.alert("提示", "文件检查未通过，请返回上一步重新上传");
                                    break;
                            }
                            return false;
                        }
                    ...
                    if (!!result.data) {
                        if (result.data.length > 0) {
                            ...
                            if ($scope.dataList.length == 0) {
                                alertify.alert("提示", "该批号对应的订单都已完成支付");
                                return false;
                            }
                            ...
                            alertify.alert("提示", "您输入的的订单批号不存在");
                        }
                    }
                });
            };
