...
/**
 * @ngdoc function
 * @name culwebApp.controller:NewOrderCtrl
 * @description
 * # NewOrderCtrl
 * Controller of the culwebApp
 */
angular.module('culwebApp') 
  .controller('NewOrderCtrl', ["$scope", "$timeout", "$filter", "orderSvr", "warehouseService", "plugMessenger",
      function ($scope, $timeout, $filter, orderSvr, warehouseService, plugMessenger) {
      this.awesomeThings = [
        'HTML5 Boilerplate',
        'AngularJS',
        'Karma'
      ...
      var _stepFunction = [
          ...
          function () {
              ...
              //})
              orderSvr.settlementForOffline($scope.dataList[0].batchNumber, function (result) {
                  $scope.paymentInfo = {
                      paymentType: "2",
                      totalWeight: 0,
                      shippingFee: 0
          ...
          function () {
              ...
              //    orderArray.push(row);
              //});
              //orderSvr.batchUpdate(orderArray, function (result) {
              //    if (!!result.message) {
      ...
      $scope.openDetail = function (item) {
          orderSvr.getDetail(item.orderNumber, function (result) {
      ...
      $scope.btnRemoveOrder = function (item) {
          plugMessenger.confirm("确定要删除吗？", function (isOK) {
              if (isOK) {
                  orderSvr.delete(item.orderNumber, function (result) {
                      if (result.success == true) {
      ...
      //    orderSvr.generatePackageNumber({
      //        "trackingNumber": "UMI",
      ...
      $scope.btnSaveDetail = function () {
          orderSvr.updateOutboundPackageAndMessage({
              "orderNumber": $scope.editOrderInfo.orderNumber,
    ...
    //   }
      ...
      $scope.btnRemovePackage = function (item) {
          plugMessenger.confirm("确定要删除该包裹吗？", function (isOK) {
              if (isOK) {
                  orderSvr.deleteOutboundPackage([item.trackingNumber], function (result) {
                      if (result.success == true) {
