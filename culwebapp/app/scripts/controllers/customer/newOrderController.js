...
angular.module('culwebApp')
    .controller('NewOrderCtrl', ["$scope", "$timeout", "$filter", "orderService", "warehouseService",
        function ($scope, $timeout, $filter, orderService, warehouseService) {
            this.awesomeThings = [
                'HTML5 Boilerplate',
                'AngularJS',
                'Karma'
            ...
            $timeout(function () {
                ...
                $wizard.on('change', function (e, data) {
                    if (data.direction == "next") {
                        if (!$scope.stepValidation[data.step - 1]) {
                            switch (data.step) {
                                case 1:
                                    alertify.alert('提示', '请上传线下订单Excel文件', 'warning');
                                    break;
                                case 2:
                                    alertify.alert('提示', '文件检查未通过，请返回上一步重新上传', 'warning');
                                    break;
                            }
                            return false;
                        }
                ...
                $('.wizard-wrapper .btn-checkout').click(function () {
                    alertify.confirm("确定要出库吗？", function (isOK) {
            ...
            $scope.btnRemoveOrder = function (item) {
                alertify.confirm("确定要删除吗？", function (isOK) {
                    if (isOK) {
            ...
            $scope.btnRemovePackage = function (item) {
                alertify.confirm("确定要删除该包裹吗？", function (isOK) {
                    if (isOK) {
                    ...
                    if (!!result.data) {
                        if (result.data.length > 0) {
                            ...
                            if ($scope.dataList.length == 0) {
                                alertify.alert('提示', '该批号对应的订单都已完成支付', 'warning');
                            ...
                            alertify.alert('提示', '您输入的的订单批号不存在', 'warning');
                        }
                    }
                });
            };
