   ...
   .directive('menuItem', ['$http', '$compile', '$state', 'menuSerivce',
       function ($http, $compile, $state, menuSerivce) {
           return {
               ...
               template: '<li ng-class="{\'list-toggle\':!!ngModel.toggle,active:ngModel.actived,\'list-group-item\':!ngModel.leaf}"  ng-click="itemSelect($event)">'
                   + '<a href="javascript:void(0);" ng-if="!ngModel.toggle"><i ng-if="!!ngModel.icon" class="{{ngModel.icon}}"></i> {{ngModel.name}}</a>'
                   + '<a href="javascript:void(0);" data-toggle="collapse" data-parent="#sidebar-nav" '
                   + 'data-target="#collapse-{{ngModel.key}}" class="collapsed" aria-expanded="false"'
                   + ' ng-if="!!ngModel.toggle"><i ng-if="!!ngModel.icon" class="{{ngModel.icon}}"></i> {{ngModel.name}}</a>'
                   + '</li>',
               link: function (scope, element, attrs) {
                   ...
                   if (scope.ngModel.visible !== false && angular.isArray(childData)) {
                       element.append('<umi-menu source="ngModel.' + childName + '" nested-inner="true" '
                           + (scope.ngModel.toggle ? 'key="collapse-' + scope.ngModel.key + '" cls="collapse" ' : '')
                           + '></umi-menu>');
           ...
           self.getPages = function (isVip) {
               ...
               },
               {
                   ...
                   childs: [{
                       ...
                       route: 'customer.sendstock'
                   }, {
                       leaf: true,//叶子节点（有父节点）
                       key: 'inventories',
                       name: '库存寄件列表',
                       icon: 'fa fa-list',
                       route: 'customer.inventories'
                   }]
